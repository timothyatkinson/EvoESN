...
void applymutate_edge_pickTarget(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel host_label1 = getNodeLabel(mutate_edge_host, host_node_index);
   if(list_length0 > 0)
   {
      HostList *list0 = makeHostList(array0, list_length0, false);
      label = makeHostLabel(host_label1.mark, list_length0, list0);
   }
   else label = makeEmptyLabel(host_label1.mark);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "mutate_edge_pickTarget", mutate_edge_host);
